...
{
    // Read existing fcse-blacklist.conf content (if any)
    QFile existing(QStringLiteral("/etc/modprobe.d/fcse-blacklist.conf"));
    QString content;
    ...
    if (content.isEmpty()) {
        content = QStringLiteral("# Generated by Fedora Core Setting Extension\n");
    }
    content += QStringLiteral("blacklist ") + name + QLatin1Char('\n');

    QVariantMap args;
    args[QStringLiteral("filePath")] = QStringLiteral("/etc/modprobe.d/fcse-blacklist.conf");
...
{
    // Read existing fcse-blacklist.conf
    QFile existing(QStringLiteral("/etc/modprobe.d/fcse-blacklist.conf"));
    if (!existing.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qWarning() << "Cannot read fcse-blacklist.conf for unblacklisting";
        return;
    }
...
K_PLUGIN_CLASS_WITH_JSON(KernelModulesKcm, "kcm_fcse_kernelmodules.json")
