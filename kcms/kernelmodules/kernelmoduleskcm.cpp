...
{
    // Read existing fls-blacklist.conf content (if any)
    QFile existing(QStringLiteral("/etc/modprobe.d/fls-blacklist.conf"));
    QString content;
    ...
    if (content.isEmpty()) {
        content = QStringLiteral("# Generated by Fedora Lowlevel Settings\n");
    }
    content += QStringLiteral("blacklist ") + name + QLatin1Char('\n');

    QVariantMap args;
    args[QStringLiteral("filePath")] = QStringLiteral("/etc/modprobe.d/fls-blacklist.conf");
...
{
    // Read existing fls-blacklist.conf
    QFile existing(QStringLiteral("/etc/modprobe.d/fls-blacklist.conf"));
    if (!existing.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qWarning() << "Cannot read fls-blacklist.conf for unblacklisting";
        return;
    }
...
K_PLUGIN_CLASS_WITH_JSON(KernelModulesKcm, "kcm_fls_kernelmodules.json")
